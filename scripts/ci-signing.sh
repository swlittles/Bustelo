...
umask 077
KEYCHAIN="$RUNNER_TEMP/bustelo-signing.keychain-db"
P12="$RUNNER_TEMP/bustelo-signing.p12"
P8="$RUNNER_TEMP/bustelo-notary.p8"
...
if [ -n "${NOTARY_API_KEY_P8:-}" ]; then
    xcrun notarytool store-credentials bustelo-notary --key "$P8" --key-id "$NOTARY_KEY_ID" --issuer "$NOTARY_ISSUER_ID" --keychain "$KEYCHAIN" >/dev/null
else
    xcrun notarytool store-credentials bustelo-notary --apple-id "$NOTARY_APPLE_ID" --password "$NOTARY_APP_PASSWORD" --team-id "${APPLE_TEAM_ID:-KQFYGC7SWB}" --keychain "$KEYCHAIN" >/dev/null
fi
printf 'BUSTELO_SIGNING_IDENTITY=%s\nNOTARY_PROFILE=bustelo-notary\n' "$IDENTITY" >> "$GITHUB_ENV"
